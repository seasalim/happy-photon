...
public partial class ImageFile : ObservableObject
{
    ...
    public ImageFile(string filePath)
    {
        ...
        IsRaw = Extension.ToUpperInvariant() switch
        {
            ".CR2" or ".CR3" or ".NEF" or ".NRW" or
            ".ARW" or ".SRF" or ".SR2" or ".DNG" or
            ".RAF" or ".ORF" or ".RW2" or ".PEF" or
            ".HEIC" or ".HEIF" => true,
            _ => false
        };
    }

    public void ReplaceThumbnail(Bitmap? thumbnail)
    {
        if (ReferenceEquals(Thumbnail, thumbnail)) return;
        var previous = Thumbnail;
        Thumbnail = thumbnail;
        previous?.Dispose();
    }

    public void DisposeThumbnail() => ReplaceThumbnail(null);
