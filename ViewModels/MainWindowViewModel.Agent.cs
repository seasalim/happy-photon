...
public partial class MainWindowViewModel
{
        ...
        int rating)
    {
        ...
        foreach (var image in images)
        {
            ...
            {
                image.Rating = rating;
                await EnsureCatalogIdAsync(image);
    ...
    {
        var failed = new List<AgentBatchFailure>();
        var succeeded = new List<ImageFile>(images.Count);
        ...
        foreach (var image in images)
        {
            ...
            try
            {
                EditSettingsTransfer.ApplySubset(copied, image.EditSettings);
        ...
        if (SelectedImage != null && succeeded.Contains(SelectedImage))
        {
            _history.Clear();
            SyncHistoryFlags();
