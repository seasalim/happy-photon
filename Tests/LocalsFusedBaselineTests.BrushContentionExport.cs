...
public sealed partial class LocalsFusedBaselineTests
{
    ...
    public async Task BrushContendedTick()
    {
        ...
        var renderer = new LocalsBrushRenderer(); var pipeline = new RenderPipeline();
        void Tick(int arm)
        {
            if (arm == 0) { using var result = pipeline.Render(new(small, control, RenderIntent.Preview, 1600, new(false, false))); }
            else { using var result = renderer.Tick(small, control, arm == 2 ? documents : null); }
        }
        ...
        for (var sample = 0; sample < Samples; sample++) for (var step = 0; step < 3; step++)
        {
            ...
            try
            {
                ...
                if (arm == 2) output.WriteLine(renderer.BuildReport);
    ...
    public async Task BrushExportDelta()
    {
        ...
        var pipeline = new RenderPipeline(); var renderer = new LocalsBrushRenderer();
        ...
        var service = new ImageExportService(pipeline, Loader(), new ExportMetadataService(),
            new DcpProfileService(new SourceAvailabilityService()), request =>
            {
                ...
                return currentArm < 2 ? pipeline.RenderDisplayRec2020(request) :
                    renderer.Display(request.Base, request.Settings, currentArm == 3 ? documents : null, RenderIntent.Export);
            });
        ...
        for (var sample = -1; sample < Samples; sample++) for (var step = 0; step < 4; step++)
        {
            ...
            file.EditSettings = currentArm == 0 ? new EditSettings() : edits;
            ...
            memory.Finish();
            if (currentArm == 3) output.WriteLine(renderer.BuildReport);
            ...
            Print(0, $"brush_export sample={sample} warmup={sample < 0} arm={new[] { "no-locals", "LH8", "brushes-off", "BCap" }[currentArm]} " +
                ...
                $"index_ms={(currentArm == 3 ? renderer.IndexMilliseconds : 0):R} kernel_ms={(currentArm >= 2 ? renderer.KernelMilliseconds : 0):R}");
        }
