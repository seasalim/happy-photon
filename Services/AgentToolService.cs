...
public sealed class AgentToolService
{
        ...
        AgentEditSettingsInput input)
    {
        ...
        {
            Exposure = Math.Clamp(input.Exposure, -3.0, 3.0),
            Temperature = Math.Clamp(input.Temperature, -100, 100),
            Brightness = Math.Clamp(input.Brightness, -100, 100),
            Contrast = Math.Clamp(input.Contrast, -100, 100),
            Saturation = Math.Clamp(input.Saturation, -100, 100),
            Vibrance = Math.Clamp(input.Vibrance, -100, 100),
            Shadows = Math.Clamp(input.Shadows, -100, 100),
            Highlights = Math.Clamp(input.Highlights, -100, 100),
            Curve = new CurveData()
        };

        return MutateAsync(ids, images =>
            _vm.ApplyColorSettingsToImagesAsync(images, settings, appliedPresetId: null));
    }
        ...
        AgentExportOptions options)
    {
        ...
        var snapshot = await OnUiThreadAsync(() =>
        {
            ...
            var requestedOutput = options.OutputFolder ?? "exports";
