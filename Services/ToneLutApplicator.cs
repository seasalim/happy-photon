...
internal static class ToneLutApplicator
{
    ...
    {
        ApplyCore(image, null, new ToneLuts(lut, lut, lut), execution);
    }
        ...
        RenderExecutionOptions? execution = null)
    {
        ...
        ApplyCore(image, matrix, luts, execution);
    }
        ...
        RenderExecutionOptions? execution = null)
    {
        ...
        Parallel.For(0, workers, options, worker =>
        {
            ...
            if (matrix == null)
            {
                for (var blockStart = start; blockStart < end;)
                {
                    execution?.ThrowIfCancellationRequested();
                    var blockEnd = blockStart + Math.Min(end - blockStart, CancellationCheckMask + 1);
                    for (var pixel = blockStart; pixel < blockEnd; pixel++)
                    {
